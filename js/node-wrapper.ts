...
const native = new Proxy({} as NativeBinding, {
  get (_target, property) {
    ...
    const value = binding[property as keyof NativeBinding]
    return typeof value === 'function' ? value.bind(binding) : value
  }
})

const capabilities = new Proxy({} as WzApiCapabilities, {
  get (_target, property) {
    return getWzCapabilities()[property as keyof WzApiCapabilities]
  }
})
...
function createWzFile (ptr: NativeHandle, ownsNative = false): WzFile {
  const NativeWzFile = WzFile as unknown as new(ptr: NativeHandle, ownsNative?: boolean) => WzFile
...
export class WzFile extends WzObject {
  ...
  static fromBlobSource (
    ...
    if (ArrayBuffer.isView(gameVersionOrMapleVersionOrIv)) {
      ...
      const callback = (readRange ?? mapleVersionOrReadRange) as BlobReadRangeCallback
      const gameVersion = mapleVersion === undefined ? -1 : gameVersionOrMapleVersionOrIv
      const version = mapleVersion === undefined
        ? gameVersionOrMapleVersionOrIv
        : mapleVersion
  ...
  constructor (pathOrPtr: string | NativeHandle, gameVersionOrMapleVersion: number | NativeIv | boolean = MapleVersion.GMS, mapleVersion?: MapleVersion) {
    ...
    if (ArrayBuffer.isView(gameVersionOrMapleVersion)) {
      ...
      const version = mapleVersion === undefined
        ? gameVersionOrMapleVersion as MapleVersion
        : mapleVersion
...
function wrapObject (type: ObjectType, ptr: NativeHandle): WzObject | null {
